...
st.info("Isi data, klik tombol di bawah, lalu PASTE gambar di draf email yang terbuka.")
...
group_list = []
current_num = 7  # Mulai penomoran grup dari nomor 7
...
for i in range(st.session_state.groups):
    with st.expander(f"Grup Data {i+1} (No. {current_num} - {current_num+2})", expanded=True):
        ...
        g_cfs = st.text_input(f"{current_num+1}. BI ID CFS", key=f"cfs_{i}") 
        ...
        current_num += 3 # Penomoran berlanjut loncat 3 angka

c1, c2 = st.columns(2)
with c1:
    st.button("➕ Tambah Grup ND", on_click=add_group, use_container_width=True)
with c2:
...
tiket_bima = st.text_input(f"{last_num}. TIKET BIMA", value="")
...
body_pesan += f"{last_num+3}. GAMBAR SS:\r\n"
body_pesan += "------------------------------------------\r\n"
body_pesan += "[ SILAKAN TEKAN CTRL+V DI SINI ]\r\n"
body_pesan += "------------------------------------------\r\n\r\n"
body_pesan += f"DARI      : {nama_hd}"

# URL Encoding (Agar karakter spesial terbaca di email)
...
# --- 5. TAMPILAN AKHIR ---
st.divider()
st.subheader("Hasil Preview Pesan:")
st.code(body_pesan)

st.link_button("🚀 BUKA GMAIL / OUTLOOK SEKARANG", mailto_link, use_container_width=True)

st.caption("Setelah email terbuka, klik pada area isi pesan lalu tekan Ctrl+V untuk memasukkan gambar.")
